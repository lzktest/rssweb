...
func tokenNext(c *gin.Context, user model.SysUser){
	...
	claims := request.CustomClaims{
		...
		BufferTime: global.GVA_CONFIG.JWT.BufferTime, // 缓冲时间1天 缓冲时间内会获得新的token刷新令牌 此时一个用户会存在两个有效令牌 但是前端之留一个 另一个会丢失
		StandardClaims: jwt.StandardClaims{
			NotBefore: time.Now().Unix() - 1000,  // 签名生效时间
			ExpiresAt: time.Now().Unix() + global.GVA_CONFIG.JWT.ExpiresTime, // 过期时间7天  配置文件
			Issuer: "tome", 	// 签名的发行者
		},
	}
	...
	if err, jwtStr := services.GetRedisJWT(user.Username); err == redis.Nil {
		if err := services.SetRedisJWT(token, user.Username); err != nil {
		...
		global.GVA_LOG.Error("设置登录状态失败", zap.Any("err", err))
		response.FailWithMessage("设置登录状态", c)
	} else {
...
func ChangePassword(c *gin.Context) {
	...
	if err, _ := services.ChangePassword(U, user.NewPassword); err != nil {
...
func GetUserList(c *gin.Context) {
	...
	if err, list, total := services.GetUserInfoList(pageInfo); err != nil {
...
func SetUserAuthority(c *gin.Context) {
	...
	if err := services.SetUserAuthority(sua.UUID, sua.AuthorityId); err != nil {
		global.GVA_LOG.Error("修改失败", zap.Any("err", err))
...
func DeleteUser(c *gin.Context) {
	...
	if err := services.DeleteUser(reqId.Id); err != nil {
...
func SetUserInfo(c *gin.Context) {
	...
	if err, ReqUser := services.SetUserInfo(user); err != nil {
